...
# Load Balancer - gRPC
# ----------------------------------------------------------------------------------------------------------------------
resource "hcloud_load_balancer" "grpc_lb" {
...
resource "hcloud_load_balancer_network" "grpc_lb" {
  load_balancer_id = hcloud_load_balancer.grpc_lb.id
  subnet_id        = hcloud_network_subnet.grpc_lb.id
  ip               = trimsuffix(cidrsubnet(hcloud_network_subnet.grpc_lb.ip_range, 8, 51), "/32")
}

resource "hcloud_load_balancer_service" "grpc_lb" {
  load_balancer_id = hcloud_load_balancer.grpc_lb.id
  protocol         = "https"
  listen_port      = "443"
...
resource "hcloud_load_balancer_target" "grpc_lb_seed" {
  type             = "label_selector"
  load_balancer_id = hcloud_load_balancer.grpc_lb.id
  label_selector   = "NodeType=seed"
  use_private_ip   = true
...
resource "hcloud_load_balancer_target" "grpc_lb_sentry" {
  type             = "label_selector"
  load_balancer_id = hcloud_load_balancer.grpc_lb.id
  label_selector   = "NodeType=sentry"
  use_private_ip   = true
