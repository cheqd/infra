...
data "hcloud_certificate" "cheqd-archive" {
  name = "${var.network}-certificate-archive"
}
...
# ----------------------------------------------------------------------------------------------------------------------
# Archive Load Balancer - RPC
# ----------------------------------------------------------------------------------------------------------------------
resource "hcloud_load_balancer" "archive_rpc_lb" {
...
resource "hcloud_load_balancer_network" "archive_rpc_lb" {
  count            = var.archive_lb ? 1 : 0

  load_balancer_id = hcloud_load_balancer.archive_rpc_lb[0].id
  subnet_id        = hcloud_network_subnet.rpc_lb.id
  ip               = trimsuffix(cidrsubnet(hcloud_network_subnet.rpc_lb.ip_range, 8, 52), "/32")
}

resource "hcloud_load_balancer_service" "archive_rpc_lb" {
  count            = var.archive_lb ? 1 : 0
  ...
  http {
    sticky_sessions = true
    redirect_http   = true
    certificates    = [data.hcloud_certificate.cheqd-archive.id]
  }
...
resource "hcloud_load_balancer_target" "archive_rpc_lb_seed" {
  count            = var.archive_lb ? 1 : 0

  type             = "label_selector"
  load_balancer_id = hcloud_load_balancer.archive_rpc_lb[0].id
  label_selector   = "Archive=true"
  use_private_ip   = true
}

# ----------------------------------------------------------------------------------------------------------------------
# Archive Load Balancer - Rest
# ----------------------------------------------------------------------------------------------------------------------
resource "hcloud_load_balancer" "archive_rest_lb" {
  count              = var.archive_lb ? 1 : 0
...
resource "hcloud_load_balancer_network" "archive_rest_lb" {
  count            = var.archive_lb ? 1 : 0

  load_balancer_id = hcloud_load_balancer.archive_rest_lb[0].id
  subnet_id        = hcloud_network_subnet.rest_lb.id
  ip               = trimsuffix(cidrsubnet(hcloud_network_subnet.rest_lb.ip_range, 8, 52), "/32")
}

resource "hcloud_load_balancer_service" "archive_rest_lb" {
  count            = var.archive_lb ? 1 : 0
...
resource "hcloud_load_balancer_target" "archive_rest_lb_seed" {
  count            = var.archive_lb ? 1 : 0
